...
// Pyodide is loaded through the local serve.py CDN proxy (/pyodide/*) so that:
//   1. All resources are served from localhost with CORP: cross-origin headers.
//   2. No dependency on CDN CORP header support (avoids COEP blocking).
//   3. Resources are disk-cached after first download; subsequent loads are instant.
const PYODIDE_INDEX_URL = "/pyodide/";
const PYODIDE_CDN = PYODIDE_INDEX_URL + "pyodide.js";
// Local wheels served by serve.py alongside index.html.
// All three packages are private — not on PyPI — so we serve them ourselves.
