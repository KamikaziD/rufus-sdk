...
async function init() {
    ...
    try {
        const probe = await fetch(
            `${self.location.origin}/dist/rufus_sdk-1.0.0rc3-py3-none-any.whl`,
            { method: "HEAD" }
        );
        if (probe.ok) {
            _wheelUrl = `${self.location.origin}/dist/rufus_sdk-1.0.0rc3-py3-none-any.whl`;
        }
    } catch (_) {
