...
def send_email(state: GreetingState, context: StepContext):
    """
    ...
    """
    print(f"[{context.step_name}] Sending Email... {state['name']}")

    return {"email_sent": True}


def random(state: GreetingState, context: StepContext):
    """
    Docstring for send_email
    ...
    :type context: StepContext
    """
    print(f"[{context.step_name}] Something Random {state}")

    return {"random_event": True}
