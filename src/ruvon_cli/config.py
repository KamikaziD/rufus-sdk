"""
Configuration management for Ruvon CLI.

Handles loading, saving, and managing CLI configuration stored in ~/.ruvon/config.yaml
"""
...
class SQLiteConfig:
    ...
    db_path: str = "~/.ruvon/workflows.db"
...
class PostgresConfig:
    ...
    db_url: str = "postgresql://localhost/ruvon"
...
@dataclass
class Config:
    """Complete Ruvon CLI configuration"""
    version: str = "1.0"
...
class ConfigManager:
    """Manages Ruvon CLI configuration"""
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        Args:
            config_path: Optional path to config file. Defaults to ~/.ruvon/config.yaml
                or $RUVON_CONFIG_DIR/config.yaml if set
        """
        if config_path is None:
            # Check for RUVON_CONFIG_DIR environment variable (used in tests)
            config_dir = os.environ.get("RUVON_CONFIG_DIR")
            if config_dir:
                self.config_path = Path(config_dir) / "config.yaml"
            else:
                self.config_path = Path.home() / ".ruvon" / "config.yaml"
        else:
    ...
    def _parse_persistence(self, data: Dict[str, Any]) -> PersistenceConfig:
        ...
        return PersistenceConfig(
            ...
            sqlite=SQLiteConfig(
                db_path=sqlite_data.get("db_path", "~/.ruvon/workflows.db"),
                auto_init=sqlite_data.get("auto_init", True)
            ),
            postgres=PostgresConfig(
                db_url=postgres_data.get("db_url", "postgresql://localhost/ruvon"),
