"""
Workflow management commands for Ruvon CLI.
...
from ruvon_cli.formatters import WorkflowListFormatter, WorkflowDetailFormatter, Formatter
        ...
        with Progress(
            ...
            while iteration < max_iterations:
                ...
                if workflow.status == 'WAITING_HUMAN':
                    ...
                    formatter.print_info(f"\nResume with:")
                    formatter.print(f"  ruvon resume {workflow_id} --input '{{\"your\": \"data\"}}' --auto")
                    break
    ...
    async def _start():
        ...
        try:
            ...
            if config_file:
                ...
                formatter.print_info(f"Example: ruvon start {workflow_type} --config workflows/{workflow_type}.yaml")
                raise typer.Exit(code=1)
            ...
            formatter.print_info(f"\nNext steps:")
            formatter.print(f"  • View details: [bold]ruvon show {workflow.id}[/bold]")
            formatter.print(f"  • Resume: [bold]ruvon resume {workflow.id}[/bold]")
