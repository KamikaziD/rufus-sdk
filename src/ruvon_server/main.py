"""
Ruvon Edge Cloud Control Plane Server.
...
# --- Import Ruvon SDK Components ---
from ruvon.engine import WorkflowEngine
from ruvon.models import WorkflowJumpDirective, WorkflowPauseDirective, WorkflowFailedException, SagaWorkflowException, HumanWorkflowStep
...
from ruvon.implementations.observability.events import EventPublisherObserver
...
# --- API Models ---
from ruvon_server.api_models import (
...
app = FastAPI(
    title="Ruvon Edge Control Plane",
    description="Cloud control plane for Ruvon Edge fintech devices",
    version="0.5.0",
    openapi_tags=[
...
_HEARTBEAT_TIMEOUT_SECONDS = int(os.getenv("RUVON_HEARTBEAT_TIMEOUT_SECONDS", "300"))
...
from ruvon_server.worker_service import WorkerService

# --- Workflow Definition + Server Command Services ---
from ruvon_server.workflow_definition_service import WorkflowDefinitionService
from ruvon_server.server_command_service import ServerCommandService

# --- WASM Component Routes ---
from ruvon_server.wasm_routes import router as wasm_router
app.include_router(wasm_router)

# --- Device Sidecar Routes ---
from ruvon_server.api.device_suggestions import router as device_suggestions_router
from ruvon_server.api.device_approvals import router as device_approvals_router
app.include_router(device_suggestions_router)
app.include_router(device_approvals_router)

# --- Policy Engine ---
from ruvon_server.policy_engine import (
...
# --- Auth / RBAC ---
from ruvon_server.auth import get_current_user, require_admin as _require_admin_orig, AuthUser as UserContext  # noqa: F401 (UserContext alias keeps existing type annotations valid)
from ruvon_server.auth.loader import load_auth_provider, set_auth_provider
...
async def require_admin(user: Optional[UserContext] = Depends(get_current_user)) -> UserContext:
    ...
    if os.getenv("RUVON_AUTH_PROVIDER", "disabled") == "disabled":
...
async def startup_event():
    ...
    RUVON_WORKFLOW_REGISTRY_PATH = os.getenv("RUVON_WORKFLOW_REGISTRY_PATH", "config/workflow_registry.yaml")
    RUVON_CONFIG_DIR = os.getenv("RUVON_CONFIG_DIR", "config")  # Directory containing workflow YAML files
    try:
        with open(RUVON_WORKFLOW_REGISTRY_PATH, "r") as f:
            workflow_registry_config = yaml.safe_load(f)
    except FileNotFoundError:
        print(f"Warning: Workflow registry not found at {RUVON_WORKFLOW_REGISTRY_PATH}.")
    ...
    if persistence_backend == 'postgres':
        ...
        DB_PATH = os.getenv('SQLITE_DB_PATH', 'ruvon_edge.db')
    ...
    from ruvon_server.version_service import VersionService
    ...
    from ruvon_server.steps.config_rollout_steps import init_services as init_rollout_services
    ...
    from ruvon_server.steps.policy_rollout_steps import init_services as init_policy_rollout_services
    init_policy_rollout_services(persistence_provider, policy_evaluator)

    # Initialize Rate Limit Service
    from ruvon_server.rate_limit_service import RateLimitService
    ...
    logger.info("Auth provider: %s", os.getenv("RUVON_AUTH_PROVIDER", "disabled"))

    # Load custom user routers (RUVON_CUSTOM_ROUTERS=my_app.routes.router,my_app.webhooks.router)
    custom_routers_env = os.getenv("RUVON_CUSTOM_ROUTERS", "").strip()
    ...
    # Initialize NATS Bridge (when RUVON_NATS_URL is set)
    nats_url = os.getenv("RUVON_NATS_URL")
    if nats_url:
        from ruvon_server.nats_bridge import NATSBridge, set_nats_bridge
        nats_bridge = NATSBridge(
            nats_url=nats_url,
            device_service=device_service,
            persistence=persistence_provider,
            nats_credentials=os.getenv("RUVON_NATS_CREDENTIALS"),
        )
    ...
    print("Ruvon Edge Control Plane started.")
...
async def shutdown_event():
    ...
    if _definition_poller_task and not _definition_poller_task.done():
        ...
        try:
            ...
            pass
    from ruvon_server.auth.loader import get_auth_provider
    ...
    from ruvon_server.nats_bridge import get_nats_bridge
    nats_bridge = get_nats_bridge()
    if nats_bridge:
        await nats_bridge.stop()
    print("Ruvon Edge Control Plane shut down.")
...
async def get_edge_impact_metrics(
    ...
    """
    Business-impact KPIs that showcase the value of Ruvon Edge:
...
async def register_device(
    ...
    expected_key = os.getenv("RUVON_REGISTRATION_KEY", "dev-registration-key")
...
async def sync_device_transactions(
    ...
    from ruvon_server.api_models import SyncAck
    return SyncResponse(
...
from ruvon_server.command_types import (
...
async def create_webhook(
    ...
    from ruvon_server.webhook_service import WebhookRegistration
...
async def get_webhook_deliveries(
    ...
    from ruvon_server.webhook_service import WebhookStatus
...
async def create_broadcast(
    ...
    from ruvon_server.broadcast import CommandBroadcast, TargetFilter, RolloutConfig
...
async def create_template(
    ...
    if not template_service:
        from ruvon_server.template_service import TemplateService
        template_service = TemplateService(persistence, device_service)

    from ruvon_server.templates import CommandTemplate
...
async def create_batch(
    ...
    if not batch_service:
        from ruvon_server.batch_service import BatchService
        batch_service = BatchService(persistence, device_service)

    from ruvon_server.batching import CommandBatch, BatchCommand, ExecutionMode
...
async def create_schedule(
    ...
    if not schedule_service:
        from ruvon_server.schedule_service import ScheduleService
    ...
    from ruvon_server.scheduling import CommandSchedule
...
async def export_audit_logs(
    ...
    from ruvon_server.audit import AuditQuery, AuditExportFormat
    from datetime import datetime
...
async def check_authorization(
    ...
    if not authorization_service:
        from ruvon_server.authorization_service import AuthorizationService
...
async def request_approval(
    ...
    from ruvon_server.authorization import ApprovalRequest
...
async def approve_command(
    ...
    from ruvon_server.authorization import ApprovalResponse
...
async def _execute_server_command(command: str, payload: dict) -> dict:
    ...
    if command == "reload_workflows":
        ...
        package = payload.get("package", "ruvon-sdk")
...
# To run: uvicorn ruvon_server.main:app --reload
