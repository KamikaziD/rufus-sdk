...
Activated when RUVON_NATS_URL is set. Falls back gracefully to HTTP if the
...
class NATSEdgeTransport:
    ...
    async def send_heartbeat(self, payload: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if _USING_PROTO:
                try:
                    from ruvon.proto.gen import HeartbeatMsg
                    msg = HeartbeatMsg(
    ...
    async def sync_transactions(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if _USING_PROTO:
                try:
                    from ruvon.proto.gen import SyncBatch, EncryptedTransaction
                    txns = []
    ...
    async def pull_config(self, etag: Optional[str]) -> Dict[str, Any]:
        ...
        try:
            from ruvon.utils.serialization import pack_message, unpack_message
