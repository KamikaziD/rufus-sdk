...
def _require_knowledge():
    ...
    try:
        from rufus.builder_ai.knowledge import KnowledgeBase
...
def rag_build(
    ...
    db_path: Optional[Path] = typer.Option(
        None, "--db",
        help="Vector store path (default: ~/.rufus/knowledge.lance)",
    ),
    ...
    Examples:
        rufus rag build                      # Incremental rebuild
        rufus rag build --force              # Full rebuild
        rufus rag build --source ./my-docs   # Include extra docs
    """
...
def rag_search(
    ...
    Examples:
        rufus rag search "how to configure HUMAN_APPROVAL step"
        rufus rag search "saga compensation pattern" --top-k 3
    """
    KnowledgeBase = _require_knowledge()
...
def rag_train_raft(
    ...
    output_model: Optional[str] = typer.Option(
        None, "--output-model",
        help="Output model name (default: rufus-expert-{hash})",
    ),
    ...
    Step 2: (unless --dry-run) Calls 'ollama create rufus-expert' to fine-tune.
    ...
    Examples:
        rufus rag train-raft --dry-run               # Generate dataset, inspect it
        rufus rag train-raft                         # Train rufus-expert from llama3
        rufus rag train-raft --base-model mistral    # Train from a different base
    """
    KnowledgeBase = _require_knowledge()

    async def _train():
        from rufus.builder_ai.knowledge.raft_dataset import RAFTDatasetGenerator
        from rufus.builder_ai.knowledge.raft_trainer import RAFTTrainer
        ...
        dataset_dir = output_dir or (Path.home() / ".rufus")
        ...
        try:
            ...
            typer.echo(
                f"\nUse it with: rufus build generate ... --backend ollama --model {final_name}"
            )
