...
Configure via env var:
  RUFUS_API_KEYS=key1:user1:role1,role2;key2:user2:admin
...
def _parse_api_keys(raw: str) -> Dict[str, AuthUser]:
    ...
    for entry in raw.split(';'):
        ...
        parts = entry.split(':')
        if len(parts) < 2:
            logger.warning('Invalid RUFUS_API_KEYS entry (expected key:user[:roles]): %s', entry)
            continue
...
class APIKeyProvider:
    def __init__(self, api_keys_env: str = ''):
        raw = api_keys_env or os.getenv('RUFUS_API_KEYS', '')
