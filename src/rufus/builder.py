...
class WorkflowBuilder:
    def build_steps_from_config(self, steps_config: List[Dict[str, Any]]):
        ...
        for config in steps_config:
            ...
            input_schema = _import_from_string(
            ...
            if step_type == "PARALLEL":
                ...
                loop_body = self.build_steps_from_config(
    ...
    def _load_registry(self):
        ...
        print("Builder: Loading registry...")
        if self._registry is None:
            try:
                ...
                raise FileNotFoundError(
                    f"Workflow registry file not found at: {self.registry_path}")
            ...
            print("Builder: Loading package dependencies...")
            self._load_package_dependencies(registry_data.get("requires", []))
            print("Builder: Finished loading package dependencies.")
        print("Builder: Registry loaded.")
    ...
    def _load_package_dependencies(self, required_packages: List[str]):
        ...
        for entry_point in importlib.metadata.entry_points(group='rufus.workflow_steps'):
            try:
                ...
                print(
                    f"Loaded step definitions from entry point: {entry_point.name} ({entry_point.value})")
            except Exception as e:
                print(
                    f"Warning: Could not load rufus.workflow_steps entry point {entry_point.name}: {e}")
            ...
            try:
                ...
                for finder, name, ispkg in pkgutil.walk_packages(module.__path__, module.__name__ + '.'):
                    try:
                        ...
                        print(
                            f"Warning: Could not import submodule {name} from package {pkg_name}: {e}")
                print(f"Loaded step definitions from package: {pkg_name}")
            except ImportError as e:
                print(
                    f"Warning: Required package '{pkg_name}' not found or failed to import: {e}")
    ...
    def get_workflow_config(self, workflow_type: str) -> Dict[str, Any]:
        ...
        if workflow_type not in self._workflow_configs:
            ...
            config_path = os.path.join(
                registry_dir, os.path.basename(config_info["config_file"]))
    ...
    def build_steps(self, workflow_type: str) -> List[WorkflowStep]:
        ...
        return self.build_steps_from_config(workflow_config.get("steps", []))
    ...
    def create_workflow(self, workflow_type: str, initial_data: Dict[str, Any] = None,
        ...
        from rufus.engine import WorkflowEngine  # Import here to avoid circular dependency
        ...
        state_model_class = _import_from_string(
        ...
        workflow_steps = self.build_steps_from_config(steps_config)
