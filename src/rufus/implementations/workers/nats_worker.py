...
Usage:
    python -m rufus.implementations.workers.nats_worker --concurrency 4

Or programmatically:
    from rufus.implementations.workers.nats_worker import NATSWorker
...
_CONSUMER_NAME = "rufus-nats-worker"
...
class NATSWorker:
    ...
    def __init__(
        ...
        self.nats_url = nats_url or os.getenv("RUFUS_NATS_URL", "nats://localhost:4222")
        self.nats_credentials = nats_credentials or os.getenv("RUFUS_NATS_CREDENTIALS")
        self.concurrency = concurrency
        self.workflow_registry_path = workflow_registry_path or os.getenv(
            "RUFUS_WORKFLOW_REGISTRY_PATH", "config/workflow_registry.yaml"
        )
        self.config_dir = config_dir or os.getenv("RUFUS_CONFIG_DIR", "config")
    ...
    async def _handle_task(self, msg) -> None:
        ...
        async with self._semaphore:
            try:
                from rufus.utils.serialization import unpack_message, pack_message
