...
class WorkflowBuilder:
    ...
    def _discover_marketplace_steps(cls):
        """
        Discovers workflow steps provided by installed `ruvon-*` marketplace packages
        ...
        # e.g., entry_points={'ruvon.steps': ['my_step = ruvon_package.steps:MyStep']}
        ...
        try:
            ...
            for entry_point in ruvon_steps:
                try:
                    ...
                    logger.error(f"Failed to load ruvon.steps entry point {entry_point.name}: {e}")
        except Exception as e:
            logger.warning(f"Could not load entry points for ruvon.steps (perhaps no packages installed?): {e}")


    @staticmethod
    def _get_merge_strategy_from_str(value: str) -> 'MergeStrategy':
        from ruvon.models import MergeStrategy # Import locally to ensure correct enum definition
    ...
    def _get_merge_conflict_behavior_from_str(value: str) -> 'MergeConflictBehavior':
        from ruvon.models import MergeConflictBehavior # Import locally to ensure correct enum definition
    ...
    def _build_steps_from_config(cls, steps_config: List[Dict[str, Any]]) -> List['WorkflowStep']:
        ...
        for config in steps_config:
            ...
            elif step_type_str == "CRON_SCHEDULE":
    ...
    async def create_workflow(self, workflow_type: str,
        ...
        # When ruvon-edge is installed, auto-detect the platform bridge so
        ...
        if wasm_binary_resolver is not None:
            ...
            try:
                from ruvon_edge.platform.wasm_bridge import detect_wasm_bridge
                bridge = detect_wasm_bridge()
            except ImportError:
                bridge = None  # cloud-only deployment — no ruvon-edge installed
    ...
    def _collect_modules_from_steps(cls, steps: List[Dict[str, Any]], modules: set):
        from ruvon.models import WorkflowStep # Import locally if not already imported
