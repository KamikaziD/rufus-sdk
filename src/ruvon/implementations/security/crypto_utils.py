...
def _get_cipher_suite():
    ...
    key = os.getenv("RUVON_ENCRYPTION_KEY")
    if not key:
        logger.warning("RUVON_ENCRYPTION_KEY not set. Generating a temporary key for this session.")
    ...
    try:
        ...
        logger.error(f"Invalid RUVON_ENCRYPTION_KEY: {e}")
        raise
