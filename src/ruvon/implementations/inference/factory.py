...
from ruvon.providers.inference import InferenceProvider, InferenceRuntime
...
class InferenceFactory:
    ...
    def get_hardware_identity(
        ...
        onnx_providers = []
        try:
            from ruvon.implementations.inference.onnx import get_available_providers
    ...
    async def _create_cpu_provider(self, **kwargs) -> InferenceProvider:
        ...
        from ruvon.implementations.inference.onnx import ONNXInferenceProvider, is_onnx_available
        from ruvon.implementations.inference.tflite import TFLiteInferenceProvider, is_tflite_available
    ...
    async def _create_onnx_provider(self, **kwargs) -> InferenceProvider:
        ...
        if "CoreMLExecutionProvider" in providers and is_apple_silicon():
            ...
            # Only set provider_options if we got actual options
            if coreml_opts:
                provider_options = [
        ...
        # Store provider options for session creation (only if not empty)
        if provider_options:
