...
Decides whether to:
  ...
  RAFT  — inject retrieved chunks; switch to the locally fine-tuned ruvon-expert model
...
class RetrievalStrategy(str, Enum):
    ...
    RAFT = "raft"    # conf ≥ 0.70 + ollama backend + ruvon-expert present
...
class RetrievalDecision(BaseModel):
    ...
    model_override: Optional[str] = None  # set to "ruvon-expert" when strategy=RAFT
...
class RAFTRouter:
    ...
    Decision algorithm:
      ...
      5. Apply thresholds:
           ...
           confidence ≥ THRESHOLD_RAFT → RAFT (if ollama + ruvon-expert available)
    ...
    def _specificity_score(self, query: str) -> float:
        ...
        q_lower = query.lower()
        hits = 0
        total = len(self.RUVON_TERMS)
        for terms in self.RUVON_TERMS.values():
    ...
    def _raft_model_available(self) -> bool:
        ...
        try:
            ...
            logger.debug("[Router] Could not check Ollama for ruvon-expert: %s", e)
