...
Usage::
    ...
    verifier = NKeyPatchVerifier.from_env()   # reads RUFUS_NKEY_PUBLIC_KEY
...
class NKeyPatchVerifier:
    ...
    def from_env(cls) -> Optional["NKeyPatchVerifier"]:
        """Create a verifier from the ``RUFUS_NKEY_PUBLIC_KEY`` environment variable.
        ...
        """
        raw = os.getenv("RUFUS_NKEY_PUBLIC_KEY", "").strip()
        if not raw:
            return None
        try:
            return cls(raw)
            ...
            logger.warning(
                "[NKey] RUFUS_NKEY_PUBLIC_KEY is set but could not be loaded: %s — "
