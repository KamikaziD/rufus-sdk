...
Returns NATSEdgeTransport when RUFUS_NATS_URL is set (or nats_url is passed),
otherwise returns HTTPEdgeTransport.
"""
import os
from typing import Optional

from rufus_edge.transport.base import EdgeTransport
from rufus_edge.transport.http_transport import HTTPEdgeTransport
...
def create_transport(
    ...
    Args:
        ...
        nats_url: NATS server URL (e.g. "nats://localhost:4222"). When provided,
                  NATSEdgeTransport is returned. Also read from RUFUS_NATS_URL env.
    ...
    """
    resolved_nats_url = nats_url or os.getenv("RUFUS_NATS_URL")

    if resolved_nats_url:
        from rufus_edge.transport.nats_transport import NATSEdgeTransport
        return NATSEdgeTransport(
            device_id=device_id,
            nats_url=resolved_nats_url,
            api_key=api_key,
            nats_credentials=nats_credentials or os.getenv("RUFUS_NATS_CREDENTIALS"),
        )
