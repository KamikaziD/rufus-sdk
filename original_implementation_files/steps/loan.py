...
from confucius.workflow import WorkflowJumpDirective, WorkflowPauseDirective, StartSubWorkflowDirective
...
@celery_app.task
def run_fraud_detection_agent(state: dict):
...
@celery_app.task
def run_full_underwriting_agent(state: dict):
    ...
    time.sleep(7) # Very long AI agent process
...
@celery_app.task
def run_simplified_underwriting_agent(state: dict):
