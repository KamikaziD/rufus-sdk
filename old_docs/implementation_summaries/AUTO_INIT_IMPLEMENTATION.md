...
**Status:** ✅ COMPLETE (Updated with Unified Migration System)
...
**Update (2026-01-31):** Auto-init now uses the **unified migration system** - both `rufus db init` and auto-init apply the same migration files, ensuring schema consistency and eliminating schema drift.
...
**Schema creation via migrations (single source of truth):**
```python
# Auto-init now uses migration files instead of embedded SQL
# migrations/003_sqlite_fixed.sql - contains complete schema
# Both CLI and auto-init apply the same migrations
```
...
**New `_create_schema()` helper (unified migration approach):**
```python
async def _create_schema(self):
    """Create database schema by applying all migrations"""
    from migrate import MigrationManager

    # Determine project root and migrations directory
...
- ✅ Creates `schema_migrations` table to track versions
- ✅ Idempotent - safe to run multiple times
- ✅ Same schema as `rufus db init` - no drift
...
**The Problem:**
Originally, auto-init used an embedded SQL schema (`SQLITE_SCHEMA` constant), while `rufus db init` used migration files. This created two sources of truth and risk of schema drift.

**The Solution:**
Both methods now use the **same migration files** (`migrations/*.sql`):

```
migrations/003_sqlite_fixed.sql (single source of truth)
                │
        ┌───────┴────────┐
        ▼                ▼
rufus db init        auto_init=True
(CLI command)    (SQLitePersistenceProvider)
        │                │
        └────────┬───────┘
                 ▼
        MigrationManager
...
1. **MigrationManager Enhanced:**
   - Added support for existing connections (`conn` parameter)
   - Added `init_fresh_database()` method with silent mode
   - Uses `executescript()` for SQLite to handle triggers properly

2. **Migration Files:**
   - `migrations/003_sqlite_fixed.sql` - Complete SQLite schema
   - No complex DEFAULT expressions (IDs generated by application)
   - Includes all tables, indexes, and triggers

3. **Path Resolution Fixed:**
   - Corrected project root calculation (5 parent directories)
   - Tools directory properly added to sys.path
...
- ✅ **Easier maintenance** - Update schema once, works everywhere
...
The auto-initialized schema is **identical** to the schema created by `rufus db init` because they both use the same migration files (`migrations/003_sqlite_fixed.sql`):
...
**What changed (Initial Implementation):**
...
**What changed (Unified Migration System Update - 2026-01-31):**
- ✅ **Migration-based initialization** - Auto-init now uses migration files
- ✅ **Single source of truth** - Both CLI and auto-init use `migrations/*.sql`
- ✅ **Version tracking** - `schema_migrations` table tracks applied versions
- ✅ **MigrationManager enhanced** - Accepts existing connections, silent mode
- ✅ **Path resolution fixed** - Correctly resolves project root (5 parents)
- ✅ **Trigger support** - Uses `executescript()` for multi-line statements
...
- ✅ **No schema drift** between auto-init and CLI init

**Files modified:**
- `src/rufus/implementations/persistence/sqlite.py` - Migration-based auto-init
- `src/rufus_cli/config.py` - Added auto_init config option
- `src/rufus_cli/providers.py` - Pass auto_init from config
- `src/rufus_cli/commands/db_cmd.py` - Use MigrationManager for init
- `tools/migrate.py` - Enhanced with connection reuse and silent mode
- `migrations/003_sqlite_fixed.sql` - Simplified migration (no complex DEFAULTs)
- `tests/cli/test_auto_init.py` - Comprehensive test suite (new file)

**Time spent:**
- Initial implementation: ~2 hours
- Unified migration system: ~3 hours

**Result:** SQLite workflows now "just work" with migration-based schema that matches CLI init exactly! 🎉
