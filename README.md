...
services:
  rufus-server:
    image: ruhfuskdev/rufus-server:1.0.0rc4
    env_file: .env
    ports: ["8000:8000"]
    depends_on: [postgres, redis]

  rufus-worker:
    image: ruhfuskdev/rufus-worker:1.0.0rc4
    env_file: .env
    volumes:
  ...
  rufus-flower:
    image: ruhfuskdev/rufus-flower:1.0.0rc4
    ports: ["5555:5555"]

  rufus-dashboard:
    image: ruhfuskdev/rufus-dashboard:1.0.0rc4
    ports: ["3000:3000"]
    environment:
...
```bash
pip install --index-url https://test.pypi.org/simple/ rufus-sdk==1.0.0rc4
```
...
The dashboard is a 9-page management UI that ships as `ruhfuskdev/rufus-dashboard:1.0.0rc4`. It connects to the REST API and provides role-based access to every aspect of a Rufus deployment.
...
- [Changelog](docs/appendices/changelog.md) — v0.1.0 → v1.0.0rc4
...
**Docker Hub:** `ruhfuskdev/rufus-server:1.0.0rc4` · `ruhfuskdev/rufus-worker:1.0.0rc4` · `ruhfuskdev/rufus-flower:1.0.0rc4` · `ruhfuskdev/rufus-dashboard:1.0.0rc4`
